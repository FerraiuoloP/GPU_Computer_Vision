...
void harrisCornerDetector(cv::Mat *img, float *sobel_x, float *sobel_y, int width, int height, float K, float ALPHA, float *gaussian_kernel_d, int FILTER_WIDTH, bool shi_tomasi)
{
    ...
    // harrisMainKernelWrap(img->data, sobel_x, sobel_y, img_harris_d, width, height, K, ALPHA, gaussian_kernel_d, FILTER_WIDTH, shi_tomasi);

    // // harris to host
    // cudaMemcpy(img_harris_h, img_harris_d, img_gray_size_h, cudaMemcpyDeviceToHost);
    // cv::imshow("Output Image!", *img);
    // cv::waitKey(0);
    // cout << "Harris corner detection done" << endl;
    ...
    // printf("Elapsed time for Max value CPU: %f ms\n", millisec);
    ...
    //                 {
    //                     img->at<cv::Vec3b>(y, x) = cv::Vec3b(240, 270, 0);
    //                 }
    //             }
    //         }
    //     }
